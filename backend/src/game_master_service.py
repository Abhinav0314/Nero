...
def load_world_data():
    try:
        with open(os.path.join(os.path.dirname(__file__), "world_data.json"), "r") as f:
            return json.load(f)
    except Exception as e:
        logger.error(f"Failed to load world_data.json: {e}")
        return {}
...
class Userdata:
    ...
    stats: Dict[str, int] = field(default_factory=lambda: {"hp": 10, "strength": 10, "dexterity": 10, "intelligence": 10})
...
async def roll_dice(
    ctx: RunContext[Userdata],
    sides: Annotated[int, Field(description="Number of sides on the die (e.g., 20)", default=20)] = 20,
    count: Annotated[int, Field(description="Number of dice to roll", default=1)] = 1,
) -> str:
    """Roll virtual dice (e.g. 1d20) and return the result."""
...
class GameMasterAgent(Agent):
    def __init__(self):
        ...
        Tone: Slightly mysterious, dramatic, but CONCISE. Avoid overly flowery language.
        Role: You are the GM. You describe scenes vividly but briefly.
        Mechanics:
            - The player has stats (HP, Strength, Dexterity, Intelligence).
            - Call for dice rolls when the player attempts something risky (e.g., "Roll a d20 for Strength").
            - Use the `roll_dice` tool to perform rolls for the world or if the player asks you to roll.
        Rules:
            - IMMEDIATELY call `start_adventure` to begin the session. Do not wait for the user to speak first.
            - Use the provided tools to start, get scene, action, journal, restart, and roll dice.
            - Keep continuity using the per-session userdata.
            - Drive short sessions. Each GM message MUST end with 'What do you do?'.
            - Speak clearly and efficiently.
        """
        super().__init__(
            instructions=instructions,
            tools=[start_adventure, get_scene, player_action, show_journal, restart_adventure, roll_dice],
        )
...
async def entrypoint(ctx: JobContext):
    ...
    # Fallback for VAD if prewarm failed
    vad = ctx.proc.userdata.get("vad")
    if not vad:
        logger.info("Loading VAD in entrypoint (fallback)...")
        try:
            vad = silero.VAD.load()
        except Exception as e:
    ...
    # Explicitly trigger the agent to start if needed, but instructions should handle it.
    # To be safe, we can send a hidden system message or just rely on the "IMMEDIATELY" instruction.
    # For now, relying on instructions as 'session.start' usually triggers the first turn if the agent is configured to speak first
    # or if the LLM decides to act.
    # To FORCE it, we can generate a response:
    asyncio.create_task(session.response.create())
